...
# Load data
file_path <- '/Users/michaeltofflemire/Mtofflemire Dropbox/Michael Tofflemire/Projects/syndiesel/scripts/results/no_migration/config_1_no_mig_1_0_10000_results/summary_output.txt'
df <- read.table(file_path, header = TRUE, sep = "\t")
...
# ---- Convert estimated means to long format ----
df_mean_long <- df %>%
  pivot_longer(
    cols = Mean_Divergence_PopA_PopB:Mean_Divergence_PopK_PopL,
    names_to = "PopPair",
    values_to = "MeanDivergence"
  ) %>%
  select(Run, PopPair, MeanDivergence, Mutation_Rate)

# ---- Convert true divergence to long format ----
df_true_long <- df %>%
  pivot_longer(
    cols = True_Divergence_PopA_PopB:True_Divergence_PopK_PopL,
...
# ---- Merge ----
df_combined <- inner_join(df_mean_long, df_true_long,
                          by = c("Run", "PopPair"))
...
# ---- Convert TRUE divergence to substitution units (μT) ----
df_combined$TrueValue_scaled <-
  df_combined$TrueValue * df_combined$Mutation_Rate
...
# Axis limit
axis_limit <- max(df_combined$MeanDivergence,
                  df_combined$TrueValue_scaled)

unique_true_vals <- unique(df_combined$TrueValue_scaled)

# ---- 95% CI ----
ci_summary <- df_combined %>%
  group_by(TrueValue_scaled) %>%
  summarise(
    Lower95 = quantile(MeanDivergence, 0.025, na.rm = TRUE),
    Upper95 = quantile(MeanDivergence, 0.975, na.rm = TRUE),
    MeanEstimate = mean(MeanDivergence, na.rm = TRUE),
    .groups = "drop"
  )

# ---- Plot ----
# ---- Plot ----
my_plot <- ggplot(df_combined,
  ...
  geom_errorbar(data = ci_summary,
                aes(x = TrueValue_scaled,
                    ymin = Lower95,
                    ymax = Upper95),
                inherit.aes = FALSE,
                width = axis_limit * 0.03,
                linewidth = 0.8,
                color = "black") +
  
  geom_point(data = ci_summary,
             aes(x = TrueValue_scaled,
                 y = MeanEstimate),
             inherit.aes = FALSE,
             size = 3,
  ...
  geom_vline(xintercept = unique_true_vals,
             linetype = "dotted",
             color = "black",
             linewidth = 0.6) +
  ...
  labs(x = expression(True~divergence~(mu*T)),
       y = expression(Estimated~divergence~(mu*T))) +
...
# 95% CI coverage
ci_coverage_df <- ci_summary %>%
  mutate(
    TrueValue = TrueValue_scaled,
    In_CI = (Lower95 <= TrueValue) &
      (TrueValue <= Upper95)
  )

coverage_prop <- mean(ci_coverage_df$In_CI)

# ---- Convert RMSE to scientific components ----
rmse_sci <- formatC(rmse_overall, format = "e", digits = 2)
parts <- strsplit(rmse_sci, "e")[[1]]

mantissa <- as.numeric(parts[1])
exponent <- as.numeric(parts[2])

# ---- Mathematical header expression ----
header_expr <- bquote(
  p(tau[e] %in% CI) == .(sprintf("%.2f", coverage_prop)) ~ "    " ~
    RMSE == .(mantissa) %*% 10^.(exponent)
)
...
my_plot <- ggplot(df_combined,
  ...
  labs(
    x = expression(True~divergence~(mu*T)),
    y = expression(Estimated~divergence~(mu*T)),
    title = header_expr
  ) +
...
output_path <- file.path(
  dirname(file_path),
  "config1_no_migration_recovery.png"
)
...
# -------------------------------------
# 1️⃣ CONSTANT MUTATION RATE
# -------------------------------------
mu <- 1e-8   # fixed across all simulations

# -------------------------------------
# 2️⃣ Path to NO MIGRATION directory
# -------------------------------------

no_mig_dir <- "/Users/michaeltofflemire/Mtofflemire Dropbox/Michael Tofflemire/Projects/syndiesel/scripts/results/no_migration"

config_dirs <- list.dirs(no_mig_dir, recursive = FALSE, full.names = TRUE)

# -------------------------------------
# 3️⃣ Loop through each config directory
# -------------------------------------

for (config_path in config_dirs) {
  
  summary_file <- file.path(config_path, "summary_output.txt")
  
  if (!file.exists(summary_file)) {
    message("Skipping (no summary file): ", basename(config_path))
    next
  }
  
  message("Processing: ", basename(config_path))
  
  # ---- Load data ----
  df <- read.table(summary_file,
                   header = TRUE,
                   sep = "\t",
                   check.names = FALSE)
  
  # Make column names unique automatically
  ...
  df_mean_long <- df %>%
    pivot_longer(
      cols = starts_with("Mean_Divergence"),
      names_to = "PopPair",
      values_to = "MeanDivergence"
    ) %>%
    select(Run, PopPair, MeanDivergence)
  ...
  df_true_long <- df %>%
    pivot_longer(
      cols = starts_with("True_Divergence"),
      names_to = "PopPair_True",
      values_to = "TrueValue"
    ) %>%
  ...
  df_combined <- inner_join(df_mean_long,
                            df_true_long,
                            by = c("Run", "PopPair")) %>%
    mutate(
      MeanDivergence = as.numeric(MeanDivergence),
  ...
  # ---- μT scaling (no mutation column used) ----
  df_combined$TrueValue_scaled <- df_combined$TrueValue * mu
  ...
  if (nrow(df_combined) == 0) {
    message("No valid data in: ", basename(config_path))
    next
  }
  ...
  # ---- Scientific formatting for RMSE ----
  
  rmse_sci <- formatC(rmse_overall,
                      format = "e",
                      digits = 2)
  ...
  output_file <- file.path(
    config_path,
    paste0(basename(config_path), "_recovery.png")
  )
...
message("All NO MIGRATION configs processed safely.")
...
# -------------------------------------------------
# 1️⃣ Path to no_migration directory
# -------------------------------------------------

no_mig_dir <- "/Users/michaeltofflemire/Mtofflemire Dropbox/Michael Tofflemire/Projects/syndiesel/scripts/results/no_migration"

# -------------------------------------------------
# 2️⃣ Collect recovery PNG files only
# -------------------------------------------------

png_files <- list.files(
  no_mig_dir,
  pattern = "_recovery\\.png$",
  recursive = TRUE,
  full.names = TRUE
)

# Remove previously combined panel
png_files <- png_files[!grepl("3x5_panel", png_files)]

# -------------------------------------------------
# 3️⃣ Extract config + split time from folder names
# -------------------------------------------------
...
file_info$config <- as.numeric(
  str_extract(file_info$file, "config_[0-9]+") %>%
    str_extract("[0-9]+")
)

# Extract split time (the FIRST large number after config)
file_info$split_time <- as.numeric(
  str_extract(file_info$file, "_[0-9]{4,6}_") %>%
    str_extract("[0-9]+")
)

# -------------------------------------------------
# 4️⃣ Define correct ordering
# -------------------------------------------------

split_order <- c(10000, 20000, 40000, 80000, 160000)

file_info <- file_info %>%
  mutate(
    config = factor(config, levels = c(1,3,6)),
    split_time = factor(split_time, levels = split_order)
  ) %>%
  arrange(config, split_time)

# Safety check
if (nrow(file_info) != 15) {
  stop("Expected 15 panels but found ", nrow(file_info))
}

# -------------------------------------------------
# 5️⃣ Convert PNGs to ggplot objects
# -------------------------------------------------

plot_list <- lapply(file_info$file, function(f) {
  img <- readPNG(f)
...
# -------------------------------------------------
# 6️⃣ Build 3 x 5 grid (NO letters)
# -------------------------------------------------
...
# -------------------------------------------------
# 7️⃣ Save high-quality composite
# -------------------------------------------------

output_file <- file.path(
  no_mig_dir,
  "no_migration_3x5_panel.png"
)
...
message("Ordered 3x5 panel saved.")
...
base_dir <- "/Users/michaeltofflemire/Mtofflemire Dropbox/Michael Tofflemire/Projects/syndiesel/scripts/results/no_migration"
...
for (cfg in configs) {
  for (sp in splits) {
    ...
    if (cfg == 1) {
      dir_name <- paste0("config_1_no_mig_1_0_", sp, "_results")
    } else {
      dir_name <- paste0("config_", cfg, "_no-mig_", cfg, "_", sp, "_10000_results")
    }
    ...
    highest_mean_event <- prob_df %>%
      group_by(EventType) %>%
      summarise(m = mean(as.numeric(Probability), na.rm=TRUE)) %>%
      filter(m == max(m)) %>%
      pull(EventType)
    ...
    prob_df <- prob_df %>%
      mutate(
        ...
        Config = factor(paste0("Config ", cfg),
                        levels = paste0("Config ", configs)),
...
# ---- RELABEL SPLITS TO 1Ne, 2Ne, 4Ne, etc ----
...
all_data$Split <- factor(all_data$Split,
                         levels = splits,
                         labels = split_labels)

fill_colors <- c(
  "True + Highest"    = "purple4",
  "True Only"         = "green",
  "Highest Mean Only" = "hotpink",
  "Other"             = "gray"
)

final_plot_prob <- ggplot(all_data,
                          aes(x = Probability,
                              y = EventType,
                              fill = FillGroup)) +
  ...
  facet_grid(Config ~ Split,
             labeller = label_parsed) +   # <- enables subscript rendering
...
ggsave("no_migration_probability_3x5_panel.png",
       final_plot_prob,
       width = 14,
       height = 8,
       dpi = 600,
  ...
  for (sp in splits) {
    ...
    prob_df <- prob_df %>%
      mutate(
        ...
        # Rename fill groups for legend clarity
        FillGroup = case_when(
          EventType == true_event & EventType == highest_mean_event ~ 
            "True # events = Highest mean probability",
          
          EventType == true_event ~ 
            "True # events",
          
          EventType == highest_mean_event ~ 
            "Highest mean probability",
...
# KEEP PANELS AS IS
# =============================
all_data$Split <- factor(all_data$Split, levels = rev(splits))
...
for (cfg in configs) {
  for (sp in splits) {
    ...
    df_combined <- inner_join(df_mean,
                              df_true,
                              by=c("Run","PopPair")) %>%
      mutate(
        MeanDivergence = as.numeric(MeanDivergence),
    ...
    ci_clean <- ci_summary %>%
      select(TrueValue_scaled, Lower95, Upper95, MeanEstimate) %>%
      mutate(
        Config = factor(paste0("NDiv=", cfg),
                        levels=paste0("NDiv=", configs)),
        Split = factor(sp, levels=splits)
    ...
    label_df <- tibble(
      ...
      label = paste0(
        "p(\u03C4\u2091 ∈ CI) = ",
        sprintf("%.2f", coverage_prop),
        "\nRMSE = ",
        formatC(rmse_overall, format="e", digits=2)
      )
    )
    
    all_points  <- bind_rows(all_points, df_points_clean)
    all_summary <- bind_rows(all_summary, ci_clean)
    panel_labels <- bind_rows(panel_labels, label_df)
  }
}

# --------------------------------------------------
# REVERSE SPLIT ORDER (match previous plot)
# --------------------------------------------------
# --------------------------------------------------
# CORRECT LEFT → RIGHT ORDER
# --------------------------------------------------
all_points$Split  <- factor(all_points$Split,  levels=splits)
all_summary$Split <- factor(all_summary$Split, levels=splits)
panel_labels$Split <- factor(panel_labels$Split, levels=splits)

# --------------------------------------------------
# CUSTOM LABELS (parsed)
# --------------------------------------------------
split_labels <- c(
  "10000"  = "1*N[e]",
  "20000"  = "2*N[e]",
...
# SAVE HIGH QUALITY
# --------------------------------------------------
ggsave("no_migration_recovery_3x5_panel.png",
       final_plot,
       width=14,
       height=8,
       dpi=600,
...
recovery_panel <- final_plot
probability_panel <- final_plot_prob
...
ggsave("no_migration_combined_6x5_panel.png",
       combined_panel,
       width = 10,
       height = 12,
       dpi = 600,
