...
MIGRATION_RATE=$4  # New migration rate argument
...
# Step 1: Update YAML files with new demographic parameters, including migration rate
echo "Updating YAML files with num_shared_events=$NUM_SHARED_EVENTS, split_time_sep=$SPLIT_TIME_SEP, baseline_split_time=$BASELINE_SPLIT_TIME, migration_rate=$MIGRATION_RATE..."
"$PYTHON_INTERPRETER" "$MAKE_YAML_SCRIPT" "$NUM_SHARED_EVENTS" "$SPLIT_TIME_SEP" "$BASELINE_SPLIT_TIME" "$MIGRATION_RATE"

# Check if YAML generation succeeded
if [ $? -ne 0 ]; then
    echo "Error: YAML generation failed."
    exit 1
fi
...
do
    ...
    # Step 3: Run the data simulation script and pass the run directory as an environment variable
    echo "Running data simulation for run ${run}..."
    SIM_RUN_DIR="$RUN_DIR" "$PYTHON_INTERPRETER" "$SIMULATE_SCRIPT"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "Simulation completed successfully for run ${run}."
    fi
    ...
    echo "Running ecoevolity for run ${run}..."
    ecoevolity --relax-triallelic-sites --prefix "${RUN_DIR}/" "$ECOEVOLITY_CONFIG"
    
    # Check if ecoevolity completed successfully
    if [ $? -ne 0 ]; then
        echo "Error: ecoevolity failed for run ${run}. Skipping further steps."
        continue
    else
        echo "Ecoevolity completed successfully for run ${run}."
    fi

    # Ensure that the state log file is present before proceeding
    if [ -f "${RUN_DIR}/ecoevolity_config-state-run-1.log" ]; then
        echo "Ecoevolity state log file found for run ${run}."

        # Step 5: Summarize chains and redirect output
        echo "Running pyco-sumchains for run ${run}..."
        pyco-sumchains -s 100 "${RUN_DIR}/ecoevolity_config-state-run-1.log" > "${RUN_DIR}/pyco-sumchains-output.txt"

        # Check if pyco-sumchains completed successfully
        if [ $? -ne 0 ]; then
            echo "Error: pyco-sumchains failed for run ${run}."
            continue
        else
            echo "pyco-sumchains completed successfully for run ${run}."
        fi

        # Step 6: Summarize ecoevolity results and redirect output
        echo "Running sumcoevolity for run ${run}..."
        sumcoevolity -b 101 -c "${ECOEVOLITY_CONFIG}" -n 1000000 "${RUN_DIR}/ecoevolity_config-state-run-1.log" > "${RUN_DIR}/sumcoevolity-results.txt"

        # Step 7: Process number of events and calculate probabilities
        echo "Calculating event probabilities for run ${run}..."
        awk '
        ...
        echo "Extracting mean divergence times for run ${run}..."
        ...
        echo "Processing number of events for run ${run}..."
        ...
        echo "Running pyco-sumtimes for run ${run}..."
...
echo "All runs completed for configuration: $NUM_SHARED_EVENTS, $SPLIT_TIME_SEP, $BASELINE_SPLIT_TIME, $MIGRATION_RATE."
